/* Formatted on 18/11/2025 10:12:18 (QP5 v5.215.12089.38647) */
  SELECT A.KODE_BANK,
         B.NAMA_BANK,
         A.TAHUN,
         A.BULAN,
    ...
    FROM (SELECT KODE_BANK,
         ...
         LEFT JOIN (SELECT A.KODE_BANK,
                           ...
                           A.JUMLAH_PEKERJA,
                           1 AS JUMLAH_DEBITUR
                      FROM SSRG_T_DEBITUR@SIKP_OLAP A
         ...
         LEFT JOIN (  SELECT KODE_BANK, NO_REK, MIN (OUTSTANDING) OUTSTANDING
                        FROM SSRG_T_TRANSAKSI@SIKP_OLAP
                    GROUP BY KODE_BANK, NO_REK) F
             ON (A.KODE_BANK = F.KODE_BANK AND A.REKENING_BARU = F.NO_REK)
         LEFT JOIN (SELECT DISTINCT NOMOR_RESI, REG_GUDANG AS KODE_GUDANG, KOMODITAS AS KODE_KOMODITAS FROM SSRG_T_RESIGUDANG@SIKP_OLAP) R
             ON (A.NO_RESI = R.NOMOR_RESI)
         LEFT JOIN (SELECT KODE_REGISTRASI, UPPER (NAMA_GUDANG) AS NAMA_GUDANG, ALAMAT_GUDANG FROM SSRG_T_GUDANG@SIKP_OLAP) S
...
--WHERE r.nomor_resi IS NOT NULL

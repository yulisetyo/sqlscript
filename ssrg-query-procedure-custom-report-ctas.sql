CREATE OR REPLACE PROCEDURE USERKUR.PROC_CUSTOM_REPORT_SSRG
AS
    v_table_exists NUMBER;
    v_ctas_query   VARCHAR2(4000);
BEGIN
    -- 1. Cek keberadaan tabel "SSRG_A_CUSTOM_REPORT"
    SELECT COUNT(*)
    INTO   v_table_exists
    FROM   ALL_TABLES
    WHERE  OWNER = 'USERKUR' -- Asumsi tabel berada di schema user yang menjalankan SP
    AND    TABLE_NAME = 'SSRG_A_CUSTOM_REPORT';

    -- 2. Jika tabel "SSRG_A_CUSTOM_REPORT" exist, lakukan perintah "DROP TABLE"
    IF v_table_exists > 0 THEN
        BEGIN
            EXECUTE IMMEDIATE 'DROP TABLE SSRG_A_CUSTOM_REPORT';
            DBMS_OUTPUT.PUT_LINE('Tabel SSRG_A_CUSTOM_REPORT berhasil dihapus.');
        EXCEPTION
            WHEN OTHERS THEN
                -- Menghindari error jika ada masalah lain saat DROP, tapi diabaikan karena tujuan utama adalah pembuatan
                DBMS_OUTPUT.PUT_LINE('Peringatan: Gagal menghapus tabel: ' || SQLERRM);
        END;
    END IF;

    -- 3. Selanjutnya lakukan perintah "CREATE TABLE SSRG_A_CUSTOM_REPORT AS SELECT ..."
    EXECUTE  IMMEDIATE 'CREATE TABLE SSRG_A_CUSTOM_REPORT AS 
      SELECT A.KODE_BANK,
             B.NAMA_BANK,
             A.TAHUN,
             A.KODE_BULAN,
             ...
             TO_CHAR (CURRENT_TIMESTAMP, ''YYYY/MM/DD HH24:MI:SS'') AS LAST_REFRESH_DATE
        FROM (SELECT KODE_BANK,
                     ...
                     TO_CHAR (TGL_AKAD, ''MM'') AS KODE_BULAN,
                     SKEMA AS KODE_SKEMA,
             ...
             LEFT JOIN (  SELECT KODE_BANK, NO_REK, MIN (OUTSTANDING) OUTSTANDING
    ...
    DBMS_OUTPUT.PUT_LINE('Tabel SSRG_A_CUSTOM_REPORT berhasil dibuat ulang.');

EXCEPTION
    WHEN OTHERS THEN
        /*-- Menangani kegagalan di level SP*/
        DBMS_OUTPUT.PUT_LINE('Error saat menjalankan prosedur: ' || SQLERRM);
        /*RAISE;*/
END PROC_CUSTOM_REPORT_SSRG;
/
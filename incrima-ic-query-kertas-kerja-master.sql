/* Formatted on 13/11/2024 14:20:40 (QP5 v5.391) */
            ...
            FROM r_faktor_dtl_butir a
                 ...
                 LEFT OUTER JOIN ( --/**ambil data reviu dokumen**/
                                  SELECT a.id_faktor, a.simpulan AS nilai
                                    FROM ic_epite_reviu_dokumen a LEFT JOIN ic_epite_rev_header b ON (a.id_reviu = b.id)
                                   WHERE b.id_program = :p_id) d
                     ON (a.id = d.id_faktor)
                 LEFT OUTER JOIN ( --/**ambil data wawancara**/
                                  SELECT a.id_faktor, a.simpulan AS nilaixxx, CASE WHEN a.simpulan = NULL THEN 0 WHEN LENGTH (a.simpulan) > 0 THEN 1 END AS nilai
                                    FROM ic_epite_wawancara a LEFT JOIN ic_epite_waw_header b ON (a.id_wawancara = b.id)
                                   WHERE b.id_program = :p_id) e
                     ON (a.id = e.id_faktor)
                 LEFT OUTER JOIN ( --/**ambil data survei**/
                                  SELECT a.id_faktor, a.simpulan AS nilai
                                    FROM ic_epite_srv_detail a LEFT JOIN ic_epite_srv_header b ON (a.id_survey = b.id)
                                   WHERE b.id_program = :p_id) f
                     ON (a.id = f.id_faktor)
                 LEFT OUTER JOIN ( --/**ambil data observasi**/
                                  SELECT a.id_faktor, a.simpulan AS nilai
                                    FROM ic_epite_observasi a LEFT JOIN ic_epite_obs_header b ON (a.id_observasi = b.id)
                                   WHERE b.id_program = :p_id) g
                     ON (a.id = g.id_faktor)
                 LEFT OUTER JOIN ( --/**ambil data yang sudah disimpan**/
                                  SELECT a.id_faktor_dtl_butir,
                                         ...
                                         b.id AS id_program
                                    FROM t_epite_hasil a LEFT OUTER JOIN t_epite b ON (a.id_epite = b.id)
                                   WHERE b.tahun = :p_tahun AND b.id = :p_id) h
                     ON (a.id = h.id_faktor_dtl_butir)) a
   WHERE id_unit IS NOT NULL
ORDER BY a.id_unit, a.id_program, a.id ASC;